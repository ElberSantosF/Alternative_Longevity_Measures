...
def test_infer_country_from_filename_returns_readable_label():
    assert infer_country_from_filename("tabua_vida_feminina_chile_2023.xlsx") == (
        "Tabua Vida Feminina Chile 2023"
    )
...
def test_load_life_table_standardizes_columns_from_csv(tmp_path):
    ...
    out = load_life_table(path, country="Teste", year=2025)

    assert list(out.columns) == ["country", "year", "age", "lx"]
    assert out["country"].tolist() == ["Teste", "Teste"]
...
def test_load_life_tables_concatenates_mapping_items(tmp_path):
    ...
    pd.DataFrame({"IDADE": [0], "lx": [100000]}).to_csv(first, index=False)
    pd.DataFrame({"IDADE": [0], "lx": [95000]}).to_csv(second, index=False)
...
def test_load_life_tables_from_metadata(tmp_path):
    ...
    pd.DataFrame({"IDADE": [0], "lx": [100000]}).to_csv(life_table, index=False)
    pd.DataFrame(
        {"filename": [str(life_table)], "country": ["Teste"], "year": [2026]}
    ).to_csv(metadata, index=False)

    loaded_metadata = load_metadata(metadata)
    out = load_life_tables_from_metadata(metadata)

    assert loaded_metadata["country"].tolist() == ["Teste"]
    assert out["country"].tolist() == ["Teste"]
